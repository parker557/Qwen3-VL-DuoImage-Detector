...
def main():
    ...
    tag_img_path = Path("/home/sensing_test/Jiang_Zheng/pic_apples.jpg")
    ...
    text_prompt = """
这是两张图片。第一张图片（pic_apple_1.jpg）展示了一个目标物体。第二张图片（pic_apples.jpg）包含了多个物体，
请分析第一张图片中的目标物体，然后在第二张图片中找出所有相同类型的物体，并为每个检测到的物体输出边界框
请只返回 JSON，不要多余解释、前后缀或 Markdown 代码块。
JSON 格式（示例值仅作结构参考）：
{
  "results": [
    { "bbox": [480,365,610,440], "label": "knife" },
    { "bbox": [580,375,775,480], "label": "knife" }
  ]
}
""".strip()
